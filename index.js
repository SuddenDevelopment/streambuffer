...
//----====|| CONFIG ||====----\\
	if(typeof objConfig === 'undefined'){
		...
		objConfig={
			...
			,newest:{size:20}
			,oldest:false
			,fnSendRecord:function(objRecord){console.log(objRecord);}
...
//----====|| METHODS ||====----\\
	...
	var fnStreamRecords=function(){
		...
		if(this.config.stream===true){ setInterval(function(){ fnStreamRecords(); }, this.config.interval); }
	};
	var fnSetViews=function(arrData){
		...
		if(arrData.length===0 && intMove===0 && this.config.stream===true){ this.config.fnEndRecords(); this.config.stream=false; }
		else{
